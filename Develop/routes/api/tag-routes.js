...
router.put('/:id', (req, res) => {
  Tag.update(req.body, {
    ...
    .then((num) => {
      if (num == 1) {
        res.send({ message: 'Tag was updated successfully.' });
      } else {
        res.send({ message: `Cannot update Tag with id=${req.params.id}. Maybe Tag was not found or req.body is empty!` });
      }
    })
    .catch((err) => {
      res.status(500).send({
  ...
  Tag.destroy({
    ...
    .then((num) => {
      if (num == 1) {
        ...
        res.send({ message: `Cannot delete Tag with id=${req.params.id}. Maybe Tag was not found!` });
      }
    })
    .catch((err) => {
      res.status(500).send({
